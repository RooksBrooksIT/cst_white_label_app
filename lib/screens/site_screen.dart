...
class _SiteScreenState extends State<SiteScreen> with SingleTickerProviderStateMixin {
  ...
  Future<List<String>> fetchProjectStatus() async {
    try {
      final snapshot = await FirestoreService.getCollection('projectStatus').get();
      return snapshot.docs
          ...
          children: [
            GlassCard(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Text('Site Information', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: GlassTextField(controller: _latitudeController, label: 'Latitude', icon: Icons.explore_outlined, readOnly: true),
                      ),
                      const SizedBox(width: 12),
                      Expanded(
                        child: GlassTextField(controller: _longitudeController, label: 'Longitude', icon: Icons.explore_outlined, readOnly: true),
                      ),
                    ],
                  ),
                ],
            ...
            GlassCard(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  Text('Project Details', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  const SizedBox(height: 16),
                  FutureBuilder<List<String>>(
                    ...
                    builder: (context, snapshot) {
                      ...
                      return _buildDropdownField(
                        value: _projectType ?? (items.isNotEmpty ? items.first : null),
                  ...
                  const SizedBox(height: 12),
                  _buildDatePicker('Start Date', _startDate, () => _selectDate(context, true)),
                  const SizedBox(height: 12),
                  _buildDatePicker('End Date', _endDate, () => _selectDate(context, false)),
                  const SizedBox(height: 12),
                  FutureBuilder<List<String>>(
          ...
          itemBuilder: (context, index) {
            ...
            return GlassCard(
              child: ListTile(
                ...
                title: Text(data['siteName'] ?? 'Unnamed Site', style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                subtitle: Text(data['location'] ?? 'No location'),
                trailing: Text(data['siteId'] ?? '', style: theme.textTheme.labelLarge?.copyWith(color: theme.primaryColor)),
              ),
            );
          },
        );
  ...
  Future<void> _saveSiteDetails() async {
    ...
    try {
      final siteId = 'ST${DateTime.now().millisecondsSinceEpoch.toString().substring(7)}';
      final data = {
        'siteId': siteId,
